...
public class PipelineManager {
  ...
  public static String startNewCascade(
  ...
  public static JobRecord registerNewJob(UpdateSpec updateSpec, JobSetting[] settings,
    ...
    JobRecord jobRecord = new JobRecord(settings, rootKey, jobInstance, params);
    if (null != parentJob) {
    ...
    for (SlotDescriptor descriptor : runBarrier.getWaitingOnInflated()) {
      ...
      if (descriptor.slot.isFilled()) {
        updateSpec.registerTask(new HandleSlotFilledTask(descriptor.slot));
      }
    }
  ...
  public static void deletePipelineRecords(String pipelineHandle)
  ...
  private static void registerReturnValue(Job<?> jobObject, Value<?> returnValue) {
    invokePrivateJobMethod(
        "registerReturnValue", jobObject, new Class[] {Value.class}, returnValue);
  }
  ...
  private static void handleExceptionDuringRun(
      JobRecord jobRecord, JobRecord rootJobRecord, UpdateSpec updateSpec, Exception e) {
  ...
  private static void finalizeJob(Key jobKey) {
    ...
    try {
      ...
      List<SlotDescriptor> finalizeSlots = finalizeBarrier.getWaitingOnInflated();
      if (null == finalizeSlots) {
        throw new RuntimeException("" + finalizeBarrier + " has not been inflated.");
      }
      int numFinalizeslots = finalizeSlots.size();
      if (1 != numFinalizeslots) {
        throw new RuntimeException("Internal logic error: numFinalizeSlots=" + numFinalizeslots);
      }
      Object value = finalizeSlots.get(0).slot.getValue();
      logger.finest("Finalizing " + jobRecord + " with value=" + value);
      ...
      logger.log(
          Level.WARNING, "An exception occurred while attempting to finalize " + jobRecord, e);
      throw new RuntimeException(e);
    }
  }
